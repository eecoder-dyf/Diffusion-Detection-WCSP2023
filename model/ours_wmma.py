...
class Network(nn.Module):
    def __init__(self, backbone, mask=False):
        ...
        self.conv_wave1 = nn.Conv2d(in_channels=3*3, out_channels=backbone.conv1.out_channels, kernel_size=3, padding=1, bias=False)
        self.conv_wave2 = nn.Conv2d(in_channels=3*backbone.conv1.out_channels, out_channels=backbone.layer2[0].conv1.in_channels, kernel_size=3, padding=1, bias=False)
        self.conv_wave3 = nn.Conv2d(in_channels=3*backbone.layer2[0].conv1.in_channels, out_channels=backbone.layer3[0].conv1.in_channels, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x, tsne=False):
        ...
        attn1 = self.conv_wave1(torch.cat([LH1, HL1, HH1], dim=1))
        wavefeat1 = self.backbone13(attn1)
        LL1 = self.convll1(LL1)
        
        feat2 = self.backbone1(feat1)
        # feat2 = self.backbone1(feat1 + attn1)
        _, _, LL2, LH2, HL2, HH2 = self.wavelet2(LL1)
        attn2 = self.conv_wave2(torch.cat([LH2, HL2, HH2], dim=1))
        ...
        attn3 = self.conv_wave3(torch.cat([LH3, HL3, HH3], dim=1))
