...
class Network(nn.Module):
    def __init__(self, backbone, mask=False):
        ...
        #     nn.ReLU(inplace=True)
        # )
        self.catlayer = nn.Conv2d(in_channels=3*backbone.layer4[0].conv1.in_channels, out_channels=backbone.layer4[0].conv1.in_channels, kernel_size=1)
    ...
    def forward(self, x, tsne=False):
        ...
        feat5 = self.catlayer(torch.cat([wavefeat1, wavefeat2, feat4], dim=1))
        ...
        if tsne:
            return fc_out, {"wavefeat1":wavefeat1, "wavefeat2":wavefeat2, "feat4":feat4}
        else:
            return fc_out
